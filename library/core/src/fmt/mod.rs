...
use crate::cell::{Cell, Ref, RefCell, RefMut, UnsafeCell, UnsafeSyncCell};
...
impl<T: ?Sized> Debug for UnsafeSyncCell<T> {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("UnsafeSyncCell").finish_non_exhaustive()
    }
}
